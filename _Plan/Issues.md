...
| I-008 | `userConfig` install-time prompts do NOT fire when the plugin is installed | bug | high | blocked | 2026-04-16 | Finding 5 Problem B | **Confirmed Claude Code bug** — two open upstream issues: [#39827](https://github.com/anthropics/claude-code/issues/39827) (variable substitution before value collection) and [#39455](https://github.com/anthropics/claude-code/issues/39455) (prompt never shown, plugin auto-enables). Affects all plugins, not just ours. No public plugin has working userConfig prompts. Our schema is correct. **Workaround:** users manually set values in `settings.json` under `pluginConfigs.dbt-pipeline-toolkit.options`. Must document in README (I-013). Blocked on upstream fix. |
...
| I-029 | sql-executor doesn't read connection from profiles.yml | enhancement | low | open | 2026-04-16 | Road Safety retrospective | `load_data.py` requires explicit `--server`/`--database` args even when a `profiles.yml` exists in the dbt project. Should fall back to parsing `profiles.yml` target connection details when CLI args are not provided and env vars are empty. |
