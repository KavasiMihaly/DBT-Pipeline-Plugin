...
| I-008 | `userConfig` install-time prompts do NOT fire when the plugin is installed | bug | high | open | 2026-04-16 | Finding 5 Problem B | Confirmed 2026-04-16: userConfig prompts do not appear during install or later. Users must manually configure connection in settings.json. Root cause unknown — may be Claude Code bug, or undocumented `title`/`type` fields. README must document manual config workaround (see I-013). |
