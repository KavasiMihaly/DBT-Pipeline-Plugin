...
tools: Agent(business-analyst, data-explorer, dbt-architecture-setup, dbt-staging-builder, dbt-dimension-builder, dbt-fact-builder, dbt-test-writer, dbt-pipeline-validator), Read, Write, Edit, Bash, Glob, Grep, TodoWrite, AskUserQuestion
model: sonnet
effort: high
color: yellow
...
You cannot be spawned as a subagent — you must be the main thread to delegate via the `Agent` tool.
...
3. Invoked you via `claude --agent dbt-pipeline-orchestrator "Build a pipeline"`
...
Spawn `business-analyst` in **foreground** (interactive):
...
Spawn `data-explorer` in **background**:
...
Spawn `dbt-architecture-setup` in **foreground** (it still runs pip/python subprocesses):
...
Spawn `dbt-staging-builder` in **background** (one at a time — sequential):
...
For each dim in Section 6: spawn `dbt-dimension-builder` in **background, parallel** (all at once):

```
Task(
  subagent_type: "dbt-dimension-builder",
...
For each fact in Section 7: spawn `dbt-fact-builder` in **background, parallel**:
...
Spawn `dbt-test-writer` in **background**:
...
Spawn `dbt-pipeline-validator` in **background**:
